...
abstract contract YieldManager is Ownable2StepUpgradeable, WithdrawalQueue, DelegateCalls {
    ...
    uint256 internal constant REPORT_YIELD_DEFAULT_GAS_LIMIT = 200_000;
    ...
    function __YieldManager_init(Portal _portal, address _owner) internal onlyInitializing {
        __Ownable2Step_init();
    ...
    function setInsurance(address _insurance, uint256 _insuranceFeeBips, uint256 _withdrawalBuffer)
        external
        onlyOwner
    {
    ...
    function commitYieldReport(bool enableInsurance) public onlyAdmin {
        ...
        for (uint256 i; i < providersLength; i++) {
            ...
            if (
                enableInsurance && YieldProvider(_providers.at(i)).supportsInsurancePayment() && insurance != address(0)
            ) {
                if (yield > 0) {
    ...
    function commitYieldReportAfterInsuranceWithdrawal(address token, uint256 amount) external onlyAdmin {
    ...
    /// @param yield Amount of yield to report.
    function _reportYield(int256 yield) internal virtual;
