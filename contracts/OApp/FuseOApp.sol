...
import {vETH} from "../fuse/vETH.sol";
...
contract FuseOApp is OApp {
    ...
    modifier onlyVETH() {
        if (msg.sender != address(VETH)) {
            revert CallerIsNotVETH();
        }
        _;
    }
    ...
    function setVETHAddress(vETH _vETH) external onlyOwner {
        require(address(_vETH) != address(0), "FuseOApp: VETH address cannot be the zero address");
        require(address(VETH) == address(0), "FuseOApp: VETH address already set");
        VETH = _vETH;
    }
    ...
    function _lzReceive(
        ...
        (MessageType messageType, address from, address recipient, uint256 amount) =
            abi.decode(payload, (MessageType, address, address, uint256));
        if (messageType == MessageType.DEPOSIT) {
            VETH.mint(recipient, amount);
        } else if (messageType == MessageType.REPORT_YIELD) {
            require(recipient == address(0), "FuseOApp: REPORT_YIELD must be sent to address(0)");
            VETH.addValue(amount);
        } else {
            revert("FuseOApp: unknown message type");
        }
    }
}
